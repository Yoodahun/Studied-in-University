...
public class LinkedSet<E> {
	...
	public boolean doesContain(E anElement) {
		...
		while(searchNode != null && !found) { //검색할 노드가 없고, 못찾을때까지 검색
			if(searchNode.element().equals(anElement)){ //노드의 원소값이 입력된 원소값과 같다면
				found = true; //빙고
			} 
	...
	public boolean add(E anElement) { //원소 저장하기
		
		if(! this.doesContain(anElement)) {
			Node<E> newNode = new Node<E>(anElement);
			//주어진 원소값의 새로운 노드를 생성
	...
	public E remove(E anElement) {
		if(this.isEmpty()){
			...
			while(currentNode != null && !found) { //지우고싶은 코인값의 노드 찾기 현재노드가 존재하고있으면서 못찾을때까지 계속
				if(currentNode.element().equals(anElement)) { //입력한 값의 원소값과 현재 원소값이 같다면
					found = true; //빙고
				} else {
	...
	public E removeAny () {
		if (this.isEmpty()) {
			...
			E removedCoin = this._head.element(); //삭제될 원소값을 저장. 삭제되는 원소값이 무엇인지 확인해야하므로
