...
public class DictionaryByBinarySearchTree<K extends Comparable<K>, O> extends Dictionary<K, O> {
	...
	private void inorderRecursively (BinaryNode<DictionaryElement<K, O>> aRootOfSubtree, int aLevel) {
	...
	public void inorder() {
		this.inorderRecursively(this._root, 1);
	}
	private void reverseOfInorderRecursively(BinaryNode<DictionaryElement<K, O>> aRootOfSubtree, int aLevel) {
		if(aRootOfSubtree != null) {
			this.reverseOfInorderRecursively(aRootOfSubtree.right(), aLevel+1);
			this.visitEvent().visitForReverseOfInorder(aRootOfSubtree.element(), aLevel);
			this.reverseOfInorderRecursively(aRootOfSubtree.left(), aLevel+1);
		}
	}
	public void reverseInorder() {
		this.reverseOfInorderRecursively(this._root, 1);
	}
